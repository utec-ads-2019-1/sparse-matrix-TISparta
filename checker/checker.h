...
class Checker {
  public:
    Checker () {}
    void isEqual (const SparseMatrix <T>& sparse_matrix, const Matrix <T>& matrix, std::string error_message) {
      ...
      for (int row = 0; row < n_rows; row++) {
        for (int column = 0; column < n_columns; column++) {
          if (sparse_matrix(row, column) != matrix(row, column)) {
            ...
            std::cout << Color::bold << Color::red << error_message << std::endl << Color::normal;
            return;
          }
        }
      }
      number_correct_tests++;
      std::cout << Color::bold << Color::green << "Test passed" << std::endl << Color::normal;
    }
